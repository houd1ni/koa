...
const { HttpError } = http_errors;
const debug = debug_('koa:application');
...
export default class Application extends Emitter {
  ...
  constructor(options) {
    ...
    if (_inspect.custom) {
      this[_inspect.custom] = this.inspect;
    }
  ...
  onerror(err) {
    ...
    if (!isNativeError) throw new TypeError(format('non-error thrown: %j', err));
